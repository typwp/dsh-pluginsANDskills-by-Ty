...
// 1. 正常迁移：qq-notify 全量
{
  ...
  check('保留 bridgeUrl 带引号', r.text.includes("bridgeUrl: 'http://localhost:3456/send'"), r.text)
  check('布尔无引号', r.text.includes('notifyApproval: true'), r.text)
  check('数字无引号', r.text.includes('approvalTimeoutMs: 900000'), r.text)
  check('Windows 路径保留', r.text.includes("'D:\\qq-bot\\harness-decisions.jsonl'"), r.text)
}
...
// 4. 特殊值：中文/冒号/空/嵌套引号（用白名单内的键）
{
  ...
  check('数组原样（YAML 数组语法）', r.text.includes('monitoredSessions: [a, b]'), r.text)
  check('引号转义（YAML 单引号翻倍）', r.text.includes("'it''''s a path'"), r.text)
}
...
// 5. 白名单剔除：context-guard 去掉 targetQq/bridgeUrl
{
  ...
  check('dropped 报告', r.dropped.includes('targetQq') && r.dropped.includes('bridgeUrl'), r.dropped.join(','))
}
