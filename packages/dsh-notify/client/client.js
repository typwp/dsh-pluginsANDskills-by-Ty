...
function showToast(message, level) {
  ...
  if (!document.getElementById('dsh-notify-style')) {
    ...
    style.textContent = '@keyframes dsh-notify-in { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }'
...
async function poll() {
  try {
    const res = await fetch(`${POLL_URL}?after=${lastSeq}`, { signal: AbortSignal.timeout(4000) })
    if (!res.ok) return
