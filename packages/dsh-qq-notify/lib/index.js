...
export function apply(ctx, config = {}) {
  ...
  async function send(message) {
    ...
    try {
      ...
      if (!res.ok) ctx.logger.warn(`[qq-notify] bridge 响应异常: ${res.status}`)
      return res.ok
    } catch (e) {
  ...
  function summarize() {
    const label = { targetQq: '目标QQ', bridgeUrl: 'bridge地址', notifyApproval: '权限通知', notifyComplete: '完成通知', notifyOnToolOnly: '仅工具回合', approvalViaQq: 'QQ审批', approvalTimeoutMs: '审批超时(ms)', monitoredSessions: '监控会话', sessionNames: '会话别名', relayNotify: '转发通知' }
    const lines = Object.entries(cfg).map(([k, v]) => {
      if (k === 'monitoredSessions') return `  ${label[k]}: ${(v ?? []).length ? v.join(', ') : '(全部)'}`
      if (k === 'sessionNames') {
      ...
      for (const line of buf.toString('utf8').split('\n')) {
        ...
        try {
          ...
          if (rec.type === 'approval' && pendingApprovals.has(rec.token)) {
            const outcome = rec.outcome === 'allowed' ? 'allowed-once' : 'rejected'
            ...
            if (rec.action === 'status') {
              ...
              if (rec.name) patch.sessionNames = { ...(cfg.sessionNames ?? {}), [rec.id]: rec.name }
              try {
                ctx.settings.mutate('qq-notify', patch)
                send(`✅ 已监控会话 ${rec.id}${rec.name ? `（${rec.name}）` : ''}`)
              } catch (e) { send(`❌ 监控设置失败: ${e.message}`) }
            } else if (rec.action === 'monitor-remove') {
              const cur = (cfg.monitoredSessions ?? []).filter((x) => x !== rec.id)
              try {
              ...
              try {
                ctx.settings.mutate('qq-notify', { sessionNames: { ...(cfg.sessionNames ?? {}), [rec.id]: rec.name } })
                send(`✅ 会话 ${rec.id} 已命名：${rec.name}`)
              } catch (e) { send(`❌ 命名失败: ${e.message}`) }
            }
          }
        } catch {}
      }
  ...
  function attachQqChannel() {
    ...
    unregisterQqChannel = notifyService.registerChannel('qq', async (message, level) => {
  ...
  ctx.on('approval/request', async (req, next) => {
    ...
    const lines = [`🔔 Harness 需要权限确认（${token}）`, `会话: ${sessionLabel(sessionId)}`, `工具: ${req.toolName}`]
    if (req.reason) lines.push(`原因: ${req.reason.slice(0, 300)}`)
    lines.push('', `在 GUI 确认，或回复「同意 ${token}」/「拒绝 ${token}」，先到先得`)
    ...
    return await new Promise((resolve) => {
      ...
      const finish = (outcome, source) => {
        ...
        clearTimeout(timer)
        if (source === 'gui') {
          const label = outcome === 'allowed-once' ? '✅ 已在 GUI 确认' : outcome === 'rejected' ? '❌ 已在 GUI 拒绝' : '已取消'
          send(`${label}（${token}），无需在 QQ 重复回复`)
        }
        resolve(outcome)
      }
      const timer = setTimeout(() => finish('rejected', 'timeout'), cfg.approvalTimeoutMs)
      qqPromise.then((o) => { if (o != null) finish(o, 'qq') })
      guiPromise.then((o) => { if (realOutcomes.has(o)) finish(o, 'gui') })
    })
  }, { prepend: true })
  ...
  ctx.on('session/event', (session, event) => {
    ...
    try {
      ...
      if (!cfg.approvalViaQq && cfg.notifyApproval && type === 'approval/asked') {
        const tool = data.toolName ?? data.name ?? ''
        const reason = typeof data.reason === 'string' ? data.reason : ''
        const detail = [tool && `工具: ${tool}`, reason && `原因: ${reason.slice(0, 200)}`].filter(Boolean).join('\n')
        send(`🔔 Harness 需要权限确认\n会话: ${sessionLabel(sid)}\n${detail || '（详见 GUI 弹窗）'}`)
        ...
        toolTurns.delete(k)
        if (data.reason?.kind === 'completed' && (!cfg.notifyOnToolOnly || hasTool)) {
          send(`✅ Harness 任务完成\n会话: ${sessionLabel(sid)}\n回合: ${data.turn}${hasTool ? '' : '（纯对话）'}`)
        }
      } else if (type === 'session/disposed') {
        for (const k of [...toolTurns.keys()]) if (k.startsWith(sid + ':')) toolTurns.delete(k)
      }
    } catch (e) {
