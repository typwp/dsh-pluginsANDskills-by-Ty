...
export function apply(ctx, config = {}) {
	...
	async function notify(message, level = cfg.notifyLevel) {
		...
		try {
			...
			ctx.logger.warn(
				`[context-guard] notify 服务调用失败，降级日志: ${e.message}`,
			);
		}
		// 降级：写入 harness 日志
	...
	function limitFor(model) {
		return cfg.modelLimits?.[model] ?? cfg.modelLimit;
	}
	...
	ctx.on("session/event", (session, event) => {
		...
		if (perSessionInput.size > 30) {
			for (const k of [...perSessionInput.keys()].slice(
				0,
				perSessionInput.size - 30,
			))
				perSessionInput.delete(k);
		}
		...
		for (const th of [...cfg.warnThresholds].sort((a, b) => a - b)) {
			...
			if (pct >= th && !warned.has(key)) {
				...
				notify(
					`⚠️ 会话 ${sid.slice(0, 8)} 上下文已达 ${Math.round(pct * 100)}%（真实输入含推理 ${input} / 上限 ${limit}）。建议开新会话交接，避免触发超限。`,
				);
			}
		}
		// 「交代后事」触发：达 handoverThreshold 时注入提醒 + 通知，让 agent 按 handover 技能收尾
		if (
			cfg.handoverThreshold > 0 &&
			pct >= cfg.handoverThreshold &&
			...
			try {
				...
				if (agent) {
					...
					ctx.logger.info(
						`[context-guard] 已向会话 ${sid.slice(0, 8)} 注入交代后事提醒`,
					);
				}
			} catch (err) {
	...
	ctx.on(
		...
		(options, next) => {
			...
			if (total > limit) {
				ctx.logger.warn(
					`[context-guard] 否决 ${sid.slice(0, 8)}: 输入 ${input} + 输出 ${budget} > ${limit}`,
				);
				notify(
					`🚫 已拦截注定失败的请求：输入 ${input} + 输出预算 ${budget} > 模型上限 ${limit}。请开新会话交接。`,
					"error",
				);
